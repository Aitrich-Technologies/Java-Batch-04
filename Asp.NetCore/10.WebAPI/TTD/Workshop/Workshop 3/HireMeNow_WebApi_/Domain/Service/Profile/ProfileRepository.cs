...
namespace Domain.Service.Profile
{
    public class ProfileRepository : IJobSeekerProfileRepository
    {
        ...
        public async Task<bool> AddProfileAsync(JobSeekerProfile profile)
        {
            profile.Id = Guid.NewGuid();
            var user = await _context.JobSeekers.Where(e => e.Id == profile.JobSeekerId).FirstOrDefaultAsync();
            if (user == null)
            {
                return false;
            }
